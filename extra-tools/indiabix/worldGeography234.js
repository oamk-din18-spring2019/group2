...
const fileName = 'indiabix-worldGeography-sec2-sec3-sec4.txt';
var differentiatingChar = '@';

async function algrorithm(startInd, lastInd, page) {
	console.log('running algorithm');
	(startInd = parseInt(startInd)), (lastInd = parseInt(lastInd));
	console.log(startInd, lastInd);
	for (var k = startInd; k <= lastInd; k++) {
		var primaryUri = 'https://www.indiabix.com/general-knowledge/world-geography/0' + k.toString().trim();
...
(async function main() {
	try {
		...
		const inds = [ [ '025001', '025014' ], [ '026001', '026014' ], [ '027001', '027013' ] ];
		for (var i = 0; i < inds.length; i++) {
			var startInd = inds[i][0];
			var lastInd = inds[i][1];
			//	console.log(startInd, lastInd);
			await algrorithm(startInd, lastInd, page);
		}
		// console.log('Done!');
		// process.exit();
	} catch (e) {
