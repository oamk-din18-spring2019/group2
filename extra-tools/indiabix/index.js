...
(async function main() {
	try {
		...
		const fileName = 'indiabix-inventions.txt';
		const differentiatingChar = '_';
		if (writeFile) {
			await fs.writeFile(
				fileName,
		...
		var startInd = parseInt('011001');
		var lastInd = parseInt('011015');
		for (var k = startInd; k <= lastInd; k++) {
			var primaryUri = 'https://www.indiabix.com/general-knowledge/inventions/0' + k.toString().trim();
